    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if itera_cnt % 100 == 0:
                    print('({}/{:.4f})'.format(itera_cnt, itera_loss / batch_cnt), end='', flush=True)
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default=R'I:\Sur3dNet\GpmmTrain', help='Dataset directory')
    parser.add_argument('--load_filename', type=str, default=None)
    parser.add_argument('--save_filename', type=str, default='sur3dnet_face_0312.pt')
...
if __name__ == '__main__':
    ...
    scheduler = (optim.lr_scheduler.LambdaLR(optimizer, last_epoch=-1, 
                                             lr_lambda=(lambda e: max(0.1 ** (int(e / 10)), 1e-3))), 
                 ext_scheduler.BNMomentum(model, last_epoch=-1, 
                                          bn_lambda=(lambda e: max(0.5 * 0.5 ** (int(e / 10)), 1e-2))))
